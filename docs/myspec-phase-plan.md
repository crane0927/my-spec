...
> 目标：将 `my-spec v2.2` 设计方案拆分为可逐阶段推进的实施任务
...
建立 `my-spec` 的最小可开发骨架，让后续功能有稳定的目录、配置和命令入口。
...
实现 MVP 最核心的执行闭环，让 `my-spec` 从“生成文档”进化到“驱动实现与验证”。
