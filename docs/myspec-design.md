...
`myspec` 参考 OpenSpec 的技术栈和工作流形态，但更强调个人质量门禁、失败回退和验证证据。
...
`myspec` 的 Markdown 和 JSON 产物应同时满足两类使用者：
...
`myspec` 允许跳过需求澄清，但应生成最小占位文档，而不是完全缺失 `clarification.md`。
...
v2.2 的核心不是增加更多命令，而是让 `myspec` 从“理念完整”变成“流程可执行、失败可恢复、结果可解释”：
...
这几项能力补齐后，`myspec` 会更像一个真正可长期使用的个人 SDD 工作流框架，而不只是一个概念上完备的设计方案。
